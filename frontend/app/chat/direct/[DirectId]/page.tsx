...
export default function DirectInterface( {params}: {params: {DirectId: string}}) {
    ...
    dispatch(setInterface("Direct"));
    dispatch(setRouteInfo(params));

    return (
        <>
            <div className={`${LeftMenuState ? 'flex w-2/3': 'flex w-full'} ${RightMenuState? 'flex w-2/3' : 'flex'}  w-2/3 border-1 mr-4 ml-4`}>
                <ChatSpace></ChatSpace>
            </div>
